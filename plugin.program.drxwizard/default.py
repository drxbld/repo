...
import xbmc, xbmcaddon, xbmcgui, xbmcplugin, os, sys, xbmcvfs, glob, time
import shutil
...
from resources.libs import control, extract, downloader, notify, debridit, traktit, loginit, skinSwitch, uploadLog, yt, wizard as wiz
...
ICONBROOM     = uservar.ICONBROOM if not uservar.ICONBROOM == 'http://' else ICON
ICONTHUMBS     = uservar.ICONTHUMBS if not uservar.ICONTHUMBS == 'http://' else ICON
ICONTRASH     = uservar.ICONTRASH if not uservar.ICONTRASH == 'http://' else ICON
ICONPACKAGE     = uservar.ICONPACKAGE if not uservar.ICONPACKAGE == 'http://' else ICON
ICONUPDATE     = uservar.ICONUPDATE if not uservar.ICONUPDATE == 'http://' else ICON
...
def index():
	...
	if not YOUTUBEFILE == 'http://' and not YOUTUBETITLE == '': addDir (YOUTUBETITLE ,'buildpreview', icon=ICONYOUTUBE, themeit=THEME1)
	...
	addDir ('Background Wizard'        ,'bgmenu',   icon=ICON,   themeit=THEME1)
...
def bgMenu():	
	...
	if not WORKINGURL == True:
		...
		addFile('[COLOR gold][B]Background Wizard[/B][/COLOR]')
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		link  = wiz.openURL(BGFILE).replace('\n','').replace('\r','').replace('\t','').replace('gui=""', 'gui="http://"').replace('theme=""', 'theme="http://"').replace('adult=""', 'adult="no"')
		match = re.compile('name="(.+?)".+?rl="(.+?)".+?escription="(.+?)"').findall(link)
		for name, url, description in match:
			addFile (name,'bg', name, url, description=description, fanart=FANART,icon=ICON, themeit=THEME2)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addFile ('Default (Single Image)','bgdefault', fanart=FANART,icon=ICON, themeit=THEME2)
		addFile ('Custom Backgrounds (.zip http:// url required)','bgcustom', fanart=FANART,icon=ICON, themeit=THEME2)
		addFile ('Clear Background Folder','bgremove', fanart=FANART,icon=ICONTRASH, themeit=THEME2)
	setView('files', 'viewType')
...
def maintMenu(view=None):
	...
	if includeall == 'true':
		...
		includenan = 'true' if INCLUDENAN     == 'true' else 'false'
		includeurl = 'true' if INCLUDEURL     == 'true' else 'false'
		includebobunleashed = 'true' if INCLUDEBOBUNLEASHED     == 'true' else 'false'
		includeelysium = 'true' if INCLUDEELYSIUM     == 'true' else 'false'
		includecovenant = 'true' if INCLUDECOVENANT     == 'true' else 'false'
	...
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('Total Clean Up: [COLOR green][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize)  ,'fullclean',       icon=ICONTRASH, themeit=THEME3)
		addFile('Clear Cache: [COLOR green][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache)     ,'clearcache',      icon=ICONBROOM, themeit=THEME3)
		addFile('Clear Packages: [COLOR green][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack)   ,'clearpackages',   icon=ICONPACKAGE, themeit=THEME3)
		addFile('Clear Thumbnails: [COLOR green][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),'clearthumb',      icon=ICONTHUMBS, themeit=THEME3)
		addFile('Clear Old Thumbnails', 'oldThumbs',      icon=ICONTHUMBS, themeit=THEME3)
		addFile('Clear Crash Logs',               'clearcrash',      icon=ICONBROOM, themeit=THEME3)
	...
	if includevid == 'true':
		...
		addFile('--- Include NaN Scrapers: %s' % includenan.replace('true',on).replace('false',off), 'togglecache', 'includenan', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Url Resolver: %s' % includeurl.replace('true',on).replace('false',off), 'togglecache', 'includeurl', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Bob Unleashed: %s' % includebobunleashed.replace('true',on).replace('false',off), 'togglecache', 'includebobunleashed', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Elysium: %s' % includeelysium.replace('true',on).replace('false',off), 'togglecache', 'includeelysium', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Include Covenant: %s' % includecovenant.replace('true',on).replace('false',off), 'togglecache', 'includecovenant', icon=ICONMAINT, themeit=THEME3)
...
def getIP():
	...
	page  = wiz.OPEN_URL(site).replace('\n','').replace('\r','')
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includenan', 'includeurl', 'includebobunleashed', 'includeelysium', 'includecovenant', 'includebob', 'includephoenix', 'includespecto', 'includegenesis', 'includeexodus', 'includeonechan', 'includesalts', 'includesaltslite']
	titlelist = ['Include Video Addons', 'Include All Addons','Include NaN Scrapers', 'Include Url Resolver', 'Include Bob Unleashed', 'Include Elysium', 'Include Covenant', 'Include Bob', 'Include Phoenix', 'Include Specto', 'Include Genesis', 'Include Exodus', 'Include One Channel', 'Include Salts', 'Include Salts Lite HD']
