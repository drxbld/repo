...
class Api(object):
	...
	def id(self, data):
		...
		data = [self.Alphabet[i] for i in data]
		return ''.join(data)
	...
	def lotteryDialog(self):
		...
		warning = 'Note that we had a minor glitch. Vouchers that were handed out to users were not written off. We therefore do not know which vouchers were already used. Hence, there is a small chance that the voucher you won was already used by someone else. In such a case, we are sorry. Better luck next time.'
		message += interface.Format.newline() + interface.Format.newline()
		message += interface.Format.bold(interface.Format.color('Important: ', color = interface.Format.ColorGaia1) + warning)
		dialog = interface.Dialog.page(title = 33879, message = message)
		while interface.Dialog.pageVisible(): tools.Time.sleep(0.5)
	...
	def donations(self, currency = None):
		parameters = {}
		if not currency == None: parameters[self.ParameterCurrency] = currency
		return self._request(type = self.TypeDonations, action = self.ActionRetrieve, parameters = parameters)

	@classmethod
	def announcements(self, last = None):
		parameters = {}
		if not last == None and not last == '': parameters[self.ParameterLast] = last
		return self._request(type = self.TypeAnnouncements, action = self.ActionRetrieve, parameters = parameters)

	@classmethod
	def deviceUpdate(self, data):
		data['identifier'] = self.idDevice()
		parameters = {}
		parameters[self.ParameterData] = tools.Converter.jsonTo(data)
		result = self._request(type = self.TypeDevice, action = self.ActionUpdate, parameters = parameters)
	...
	def speedtestAdd(self, data):
		...
		parameters = {}
		parameters[self.ParameterData] = tools.Converter.jsonTo(data)
		result = self._request(type = self.TypeSpeedtest, action = self.ActionAdd, parameters = parameters)
		self._lotteryUpdate(result)

	@classmethod
	def speedtestRetrieve(self, service, selection, continent, country, region, city):
		parameters = {}
		if not service == self.ServiceNone: parameters[self.ParameterService] = service
		parameters[self.ParameterSelection] = selection
		parameters[self.ParameterContinent] = continent
		parameters[self.ParameterCountry] = country
		parameters[self.ParameterRegion] = region
		parameters[self.ParameterCity] = city
		return self._request(type = self.TypeSpeedtest, action = self.ActionRetrieve, parameters = parameters)

	@classmethod
	def flare(self, link):
		parameters = {}
		parameters[self.ParameterLink] = link
		return self._request(type = self.TypeFlare, parameters = parameters, raw = True)
